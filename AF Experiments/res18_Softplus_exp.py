...
res18_model = models.resnet18(pretrained = False)
...
def replace_relu_to_Softplus(model):
    for child_name, child in model.named_children():
        if isinstance(child, nn.ReLU):
            setattr(model, child_name, nn.Softplus())
        else:
            replace_relu_to_Softplus(child)

replace_relu_to_Softplus(res18_model)

# Criterion and optimizer;
# Only SGD is used for activation function experiments.
...
if __name__ == "__main__":
  ...
  res18_Softplus_model, res18_Softplus_epochs, res18_Softplus_best_acc = train_model(res18_model, dataloaders_dict, criterion, res18_sgd, 100)

  # Write the epoch losses and accuracy into .txt files
  with open("res18_Softplus_best_acc.txt", "w") as f:
      f.write(f"{res18_Softplus_best_acc}")

  with open("res18_Softplus_losses.txt", "w") as f:
      for item in res18_Softplus_epochs[0]:
          f.write(f"{item}\n")

  with open("res18_Softplus_accs.txt", "w") as f:
      for item in res18_Softplus_epochs[1]:
          f.write(f"{item}\n")