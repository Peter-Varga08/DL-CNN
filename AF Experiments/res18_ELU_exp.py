...
# Recursively replace ReLU with ELU
def replace_relu_to_ELU(model):
    for child_name, child in model.named_children():
        if isinstance(child, nn.ReLU):
            setattr(model, child_name, nn.ELU())
        else:
            replace_relu_to_ELU(child)

replace_relu_to_ELU(res18_model)
...
if __name__ == "__main__":
  ...
  res18_ELU_model, res18_ELU_epochs, res18_ELU_best_acc = train_model(res18_model, dataloaders_dict, criterion, res18_sgd, 100)

  # Write the epoch losses and accuracy into .txt files
  with open("res18_ELU_best_acc.txt", "w") as f:
      f.write(f"{res18_ELU_best_acc}")

  with open("res18_ELU_losses.txt", "w") as f:
      for item in res18_ELU_epochs[0]:
          f.write(f"{item}\n")

  with open("res18_ELU_accs.txt", "w") as f:
      for item in res18_ELU_epochs[1]:
          f.write(f"{item}\n")